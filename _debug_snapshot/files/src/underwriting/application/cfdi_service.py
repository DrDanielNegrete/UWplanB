...
class CfdiService:
    ...
    def _build_counterparty_tables(self, taxpayer_rfc: str, emit_items: List[Dict[str, Any]], rec_items: List[Dict[str, Any]]) -> tuple[pd.DataFrame, pd.DataFrame]:
        """
        ...
        def _agg_for_counterparty(counter_rfc: str, counter_name: str) -> Dict[str, Any]:
            ...
            if not e_I_vig.empty and "paymentType" in e_I_vig.columns:
                ...
                if not e_ppd.empty and "issuedAt" in e_ppd.columns and e_ppd["issuedAt"].notna().any():
                    # toma la mejor fecha de pago disponible
                    pay_dt = None
    ...
    def fetch_syntage_xml(self, rfc: str, *, date_from: date | None, date_to: date | None):
        ...
        h1, c1 = self.parser.parse_many(emit_xmls)  # ✅ bytes
        h2, c2 = self.parser.parse_many(rec_xmls)   # ✅ bytes

        # ✅ NUEVO: construir tablas clientes/proveedores usando metadata de list_invoices
        clientes_df, proveedores_df = self._build_counterparty_tables(rfc, emit, rec)
    ...
    def prodserv_summary_shiny(
        ...
        *,
        ...
        out_cols = ["clave_prodserv", "producto", "conteo", "monto"]
